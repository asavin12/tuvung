...
// Token mã hóa base64 (thay bằng base64 của GITHUB_TOKEN)
const ENCODED_GITHUB_TOKEN = "Z2hwX0k0WHpKOWJKUXpkd1ZsV1NoYTlHWU9leXlsSThUajFmTXg1VA==";
...
async function loadVocabList() {
    const token = decodeToken(ENCODED_GITHUB_TOKEN);
    if (!token) {
        alert("Token không hợp lệ. Vui lòng kiểm tra ENCODED_GITHUB_TOKEN.");
        return;
    }
    ...
    try {
        ...
        alert(`Không thể tải danh sách từ vựng: ${error.message}. Vui lòng kiểm tra console, token, và kết nối mạng.`);
    }
}
...
async function saveVocabList() {
    ...
    try {
        ...
        alert(`Không thể lưu danh sách từ vựng: ${error.message}. Vui lòng kiểm tra console, token, và kết nối mạng.`);
    }
}
